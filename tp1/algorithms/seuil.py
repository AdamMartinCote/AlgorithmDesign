...
from algorithms.common import distance
...
def seuil(points_x, points_y, seuil_recur):
    """
    ...
    min_left = seuil(left_group_x, left_group_y, seuil_recur)
    min_right = seuil(right_group_x, right_group_y, seuil_recur)
...
def execute_DpR(sorted_points_x, sorted_points_y, seuil_recur):
    start = time.time()
    min_dpr = seuil(sorted_points_x, sorted_points_y, seuil_recur)
    end = time.time()
    # print("DPR: ", min_dpr)
    return end - start
